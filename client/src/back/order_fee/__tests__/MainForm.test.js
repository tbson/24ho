...
describe('Service.changeRequest', () => {
    it('No ID', async () => {
        ...
        expect(apiCall.mock.calls[0][0]).toEqual('http://localhost/api/v1/order-fee/');
    ...
    it('With ID', async () => {
        ...
        await Service.changeRequest(params);
        expect(apiCall).toHaveBeenCalled();
        expect(apiCall.mock.calls[0][0]).toEqual(`http://localhost/api/v1/order-fee/${id}`);
        expect(apiCall.mock.calls[0][1]).toEqual(params);
...
describe('Service.validate', () => {
    test('From > To', async () => {
        ...
        const eput = {from_amount: ErrMsgs.RANGE};
...
describe('Service.validationSchema', () => {
    ...
    test('Fail', () => {
        ...
        const eput = {
            from_amount: [ErrMsgs.REQUIRED],
            to_amount: [ErrMsgs.GT_0]
        };
        let output = {};
        try {
