...
describe('Service.changeRequest', () => {
    it('No ID', async () => {
        ...
        expect(apiCall.mock.calls[0][0]).toEqual('http://localhost/api/v1/customer_group/');
    ...
    it('With ID', async () => {
        ...
        await Service.changeRequest(params);
        expect(apiCall).toHaveBeenCalled();
        expect(apiCall.mock.calls[0][0]).toEqual(`http://localhost/api/v1/customer_group/${id}`);
        expect(apiCall.mock.calls[0][1]).toEqual(params);
