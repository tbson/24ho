...
export class CustomerTable extends React.Component<Props, States> {
    nextUrl: ?string;
    prevUrl: ?string;
    ...
    render() {
        ...
        const modalTitle = formValues.id ? 'Update customer' : 'Add new customer';
        return (
            <div>
                ...
                <DefaultModal open={this.state.modal} title={modalTitle} handleClose={() => this.toggleModal('modal')}>
                    <CustomerForm formValues={formValues} formErrors={formErrors} groupList={groupList} handleSubmit={this.handleSubmit}>
