...
describe('Service.changeRequest', () => {
    it('No ID', async () => {
        ...
        expect(apiCall.mock.calls[0][0]).toEqual('http://localhost/api/v1/count-check/');
    ...
    it('With ID', async () => {
        ...
        await Service.changeRequest(params);
        expect(apiCall).toHaveBeenCalled();
        expect(apiCall.mock.calls[0][0]).toEqual(`http://localhost/api/v1/count-check/${id}`);
        expect(apiCall.mock.calls[0][1]).toEqual(params);
...
describe('Service.validate', () => {
    test('From > To', async () => {
        ...
        const eput = {from_items: ErrMsgs.RANGE};
...
describe('Service.validationSchema', () => {
    ...
    test('Fail', () => {
        const values = {
            from_items: undefined,
            to_items: -1,
            fee: 3
        };
        const eput = {
            from_items: [ErrMsgs.REQUIRED],
            to_items: [ErrMsgs.GT_0]
        };
        let output = {};
        try {
