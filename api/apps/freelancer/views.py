...
class FreelancerViewSet(GenericViewSet):
    permissions = (
        'list_freelancer',
        'retrieve_freelancer',
        'add_freelancer',
        'change_freelancer',
        'delete_freelancer',
        'delete_list_freelancer',
    )
    name = 'freelancer'
    permission_classes = (CustomPermissionExp, )
    serializer_class = FreelancerBaseSerializer
    ...
    def list(self, request):
        ...
        serializer = FreelancerBaseSerializer(queryset, many=True)
        return self.get_paginated_response(serializer.data)

    def retrieve(self, request, pk=None):
        obj = get_object_or_404(Freelancer, pk=pk)
        serializer = FreelancerBaseSerializer(obj)
    ...
    def change(self, request, pk=None):
        obj = get_object_or_404(Freelancer, pk=pk)
        serializer = FreelancerUpdateSerializer(obj, data=request.data)
        if serializer.is_valid(raise_exception=True):
            serializer.save()
        return Response(serializer.data)
    ...
    def delete_list(self, request):
        ...
        result = Freelancer.objects.filter(pk__in=pk)
        if result.count() == 0:
            raise Http404
        result.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
...
class LoginView(ObtainJSONWebToken):
    def post(self, request, *args, **kwargs):
        ...
        if response.status_code == 200 and response.data['user']['table'] == 'freelancers':
            return response
...
class ProfileView(APIView):
    ...
    def get(self, request, format=None):
        ...
        repr(user)
        serializer = FreelancerBaseSerializer(self.get_object().freelancer)
        return Response(serializer.data)

    def post(self, request, format=None):
        params = self.request.data
        freelancer = self.get_object().freelancer
        serializer = FreelancerUpdateSerializer(freelancer, data=params, partial=True)
...
class ResetPasswordView(APIView):
    ...
    def get_object(self, queryStr, type="username"):
        try:
            if type == "username":
                return Freelancer.objects.get(user__username=queryStr)
            elif type == "reset_password_token":
                return Freelancer.objects.get(reset_password_token=queryStr)
            elif type == "change_password_token":
                return Freelancer.objects.get(change_password_token=queryStr)
            else:
                raise Http404
    ...
    def post(self, request, format=None):
        ...
        item.save()
        url = settings.BASE_URL + "freelancer/reset-password/" + str(token)
